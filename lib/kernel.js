...
weight_vectors = function(kernel, bandwidth, x_0, x_i){
...
matrixize = function(fun){
  return function(X){
    if(_.isArray(X) === true){
      if(_.isArray(X[0]) === false){
        return fun(X);
      } else {
...
ns.silverman = function(x){
  var num = 4 * Math.pow(jStat.stdev(x), 5);
...
ns.multipleRegression = function(Xs, ys, kernel, bandwidth){
  ...
  if(_.isArray(Xs) === false || _.isArray(Xs[0]) === false){
  ...
  var kernel_smoother = function(x){
    ...
    if(_.isArray(x) === false || x.length != _p){
